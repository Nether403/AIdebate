...
- Cost ceilings are enforced from real recorded cost: per-debate and per-run ceilings (`benchmark_runs.config.perDebateCostCeilingUsd` / `perRunCostCeilingUsd`) trip offending debates to `evaluation_failed`, and the daily spending cap now reflects real accumulated spend over the current UTC day (the previous `currentSpend = 0` stub is fixed) and fails closed if spend cannot be computed. Because cost is sourced from `llm_provider_calls.cost_estimate`, BYOK-routed/Azure `$0` calls contribute `$0` to these totals — a run dominated by BYOK/Azure models can under-count true upstream spend (see the cost-telemetry caveat above).
- The `includeEvaluationFailed` opt-in exists on `buildModelMetrics`, but the `app/api/monitoring/metrics` and `app/api/admin/metrics` routes do not currently compute per-model win/loss/tie metrics (the only caller of `buildModelMetrics` is `scripts/export-dataset.ts`), so there is no HTTP query parameter wired for it yet.
